...
extension UINavigationBar:WRAwakeProtocol
{
    ...
    fileprivate var backgroundImageView: NavigationBackgroundView? {
        get {
            guard let bgImageView = objc_getAssociatedObject(self, &AssociatedKeys.backgroundImageView) as? NavigationBackgroundView else {
                return nil
            }
            return bgImageView
        }
    ...
    fileprivate func wr_setBackgroundImage(image:UIImage)
    {
        if (backgroundImageView == nil)
        {
            ...
            backgroundImageView = NavigationBackgroundView(frame: CGRect(x: 0, y: 0, width: Int(bounds.width), height: WRNavigationBar.navBarBottom()))
        ...
        backgroundImageView?.updateImageView(image)
    }
    
    // set navigationBar backImage
    fileprivate func wr_setBackImage(image:UIImage)
    {
...
extension UINavigationController: WRFatherAwakeProtocol
{
    ...
    fileprivate func setNeedsNavigationBarUpdate(backgroundImage: UIImage?)
    {
        ...
        navigationBar.wr_setBackgroundImage(image: image)
    }
    
    fileprivate func setNeedsNavigationBarUpdate(to: UIImage, progress: CGFloat)
    {
        navigationBar.backgroundImageView?.change(to: to, progress: progress)
    }
    
    fileprivate func setNeedsNavigationBarUpdate(backImage: UIImage)
    {
        navigationBar.wr_setBackImage(image: backImage)
    }
    ...
    {
        navigationBar.shadowImage = (hideShadowImage == true) ? UIImage() : WRNavigationBar.defaultNavBarShadowImage
    }
    ...
    {
        // 改变背景图片
//        let fromBarBackgroundImage = fromVC?.navBarBackgroundImage ?? WRNavigationBar.defaultNavBarBackgroundImage
        if let toBarBackgroundImage   = toVC?.navBarBackgroundImage ?? WRNavigationBar.defaultNavBarBackgroundImage {
            setNeedsNavigationBarUpdate(to: toBarBackgroundImage, progress: progress)
        }
        
        // change navBarBarTintColor
//        let fromBarTintColor = fromVC?.navBarBackgroundImage ?? WRNavigationBar.defaultNavBarBackgroundImage
//        let toBarTintColor   = toVC?.navBarBackgroundImage ?? WRNavigationBar.defaultNavBarBackgroundImage
...
extension UINavigationController: UINavigationBarDelegate
{
    public func navigationBar(_ navigationBar: UINavigationBar, shouldPop item: UINavigationItem) -> Bool
    {
        ...
        if let currentVCDelegate = self.topViewController as? ShouldPopDelegate {
            shouldPop = currentVCDelegate.currentViewControllerShouldPop()
        }
        ...
        if (shouldPop == true)
        {
            DispatchQueue.main.async {
                ...
                if let topVC = self.topViewController,
                    ...
                    let popToVC = self.viewControllers[self.viewControllers.count - n]
    ...
    private func dealInteractionChanges(_ context: UIViewControllerTransitionCoordinatorContext)
    {
        let animations: (UITransitionContextViewControllerKey) -> () = {
            let curImage = context.viewController(forKey: $0)?.navBarBackgroundImage ?? WRNavigationBar.defaultNavBarBackgroundImage
            let curAlpha = context.viewController(forKey: $0)?.navBarBackgroundAlpha ?? WRNavigationBar.defaultBackgroundAlpha
            
            self.setNeedsNavigationBarUpdate(backgroundImage: curImage)
...
extension UIViewController: WRAwakeProtocol
{
    fileprivate struct AssociatedKeys
    {
        ...
        static var navBarBackImage: UIImage = UIImage()
    ...
    // you can set navigationBar back image
    var navBarBackImage: UIImage?
    {
        get {
            guard let bgImage = objc_getAssociatedObject(self, &AssociatedKeys.navBarBackImage) as? UIImage else {
                return WRNavigationBar.defaultNavBarDarkBackImage
            }
            return bgImage
        }
        set {
            objc_setAssociatedObject(self, &AssociatedKeys.navBarBackImage, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }

    }
    
    // navigationBar barTintColor 导航栏背景颜色
...
class WRNavigationBar
{
    ...
    {   // default is system attributes
        ...
        static var defNavBarBackShadowImage: UIImage = UIImage()
        ...
        static var defNavBarWhiteBackImage: UIImage = UIImage()
        static var defNavBarDarkBackImage: UIImage = UIImage()
    }
    ...
    class var defaultNavBarShadowImage: UIImage? {
        get {
            guard let def = objc_getAssociatedObject(self, &AssociatedKeys.defNavBarBackShadowImage) as? UIImage else {
                return nil
            }
            return def
        }
        set {
            objc_setAssociatedObject(self, &AssociatedKeys.defNavBarBackShadowImage, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    ...
    class var defaultNavBarWhiteBackImage: UIImage? {
        get {
            guard let def = objc_getAssociatedObject(self, &AssociatedKeys.defNavBarWhiteBackImage) as? UIImage else {
                return nil
            }
            return def
        }
        set {
            objc_setAssociatedObject(self, &AssociatedKeys.defNavBarWhiteBackImage, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }

    
    class var defaultNavBarDarkBackImage: UIImage? {
        get {
            guard let def = objc_getAssociatedObject(self, &AssociatedKeys.defNavBarDarkBackImage) as? UIImage else {
                return nil
            }
            return def
        }
        set {
            objc_setAssociatedObject(self, &AssociatedKeys.defNavBarDarkBackImage, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
